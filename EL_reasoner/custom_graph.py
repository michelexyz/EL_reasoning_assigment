...
class Node:
    ...
    def add_concept(self, concept):
        if concept not in self.concepts:
            ...
            return False
    # def add_initial_concept(self, concept):
    #     if concept not in self.initial_concepts:
    #         self.initial_concepts.add(concept)
    #         print(f"Node.add_initial_concept: Added concept {concept} to node {self.name}")
    #         return True
    #     else:
    #         print(f"Node.add_initial_concept: Concept {concept} already in node {self.name}")
    #         return False
...
class Graph:
    ...
    def add_relation(self, rel_name: str, from_node: str, to_node: str):
        """
        ...
        if rel_name not in from_node.successor_relations.keys():
            self.successor_relations[rel_name] = Relation(rel_name)
        if rel_name not in to_node.predecessor_relations.keys():
            self.predecessor_relations[rel_name] = Relation(rel_name)

        bool_successor = from_node.successor_relations[rel_name].add_edge(to_node.name)
        bool_predecessor = to_node.predecessor_relations[rel_name].add_edge(from_node.name)
        assert bool_successor == bool_predecessor, "Graph.add_relation: successor and predecessor relations should be added in pairs"
        if bool_successor:
            print(f"Graph.add_relation: Added relation {rel_name} from node {from_node.name} to node {to_node.name}")
